...
class Ledger_purchase_ajax extends BaseController
{
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            $mBalanceSubquery = "SUM(
                CASE 
                    ...
                    ELSE 0 
                END
            ) OVER (ORDER BY ledgPurch_id) AS r_balance";
            // 2. Build the inner subquery
            $subquery = $db->table('ledger_purchase')
                ->select('ledger_purchase.*')
            ...
            $table->orderBy('ledgPurch_id', 'ASC');
            $data['ledger_purchase'] = $table->get()->getResult();
