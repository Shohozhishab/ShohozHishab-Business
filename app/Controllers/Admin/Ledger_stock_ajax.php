...
class Ledger_stock_ajax extends BaseController
{
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            $mBalanceSubquery = "SUM(
                    CASE 
                        ...
                        ELSE 0 
                    END
                ) OVER (ORDER BY stock_id) AS r_balance";
            // 2. Build the inner subquery
            $subquery = $db->table('ledger_stock')
                ->select('ledger_stock.*')
            ...
            $table->orderBy('stock_id', 'ASC');
            $data['ledger_stock'] = $table->get()->getResult();
