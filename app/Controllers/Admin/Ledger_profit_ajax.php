...
class Ledger_profit_ajax extends BaseController
{
    ...
    protected $session;
    protected $crop;
    private $module_name = 'Ledger_profit';
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            $mBalanceSubquery = "SUM(
                    CASE 
                        ...
                        ELSE 0 
                    END
                ) OVER (ORDER BY profit_id) AS r_balance";
            // 2. Build the inner subquery
            $subquery = $db->table('ledger_profit')
                ->select('ledger_profit.*')
            ...
            $table->orderBy('profit_id', 'ASC');
            $data['ledger_profit'] = $table->get()->getResult();
            ...
            if (isset($data['mod_access']) and $data['mod_access'] == 1) {
                echo view('Admin/Ledger_profit/list', $data);
            } else {
