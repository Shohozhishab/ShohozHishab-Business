...
class Ledger_discount_ajax extends BaseController
{
        ...
        if (!isset($isLoggedIn) || $isLoggedIn != TRUE) {
            ...
            $mBalanceSubquery = "SUM(
                    CASE 
                        ...
                        ELSE 0 
                    END
                ) OVER (ORDER BY discount_ledg_id) AS r_balance";
            // 2. Build the inner subquery
            $subquery = $db->table('ledger_discount')
                ->select('ledger_discount.*')
            ...
            $table->orderBy('discount_ledg_id', 'ASC');
